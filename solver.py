...
try: 
    from deps import TensorboardWriter
except:
        ...
        for epoch_i in trange(self.config.n_epochs, desc='Epoch', ncols=80):
            ...
            for batch_i, image_features in enumerate(tqdm(
                ...
                if image_features.size(1) > 10000:
                    continue
                print(f"Image_features shape: {image_features.size()}")
                ...
                # Maximization
                c_loss = -1.0 * self.gan_loss(original_prob, fake_prob, uniform_prob)

                tqdm.write(f'gan loss: {gan_loss.item():.3f}')
                tqdm.write(f'c_loss loss: {-1.0 * self.gan_loss(original_prob, fake_prob, uniform_prob):.3f}')
                ...
                tqdm.write(f"C_loss = {c_loss}\nC_loss item = {c_loss.item()}")
                c_loss_history.append(c_loss)
                print(f"c_loss_history: {c_loss_history}")
