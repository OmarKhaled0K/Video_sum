...
class Solver(object):
    ...
    def build(self):
        ...
        self.summarizer = Summarizer(input_size=self.config.hidden_size, hidden_size=self.config.hidden_size, num_layers=self.config.num_layers).cuda()
        
        self.discriminator = Discriminator(input_size=self.config.hidden_size, hidden_size=self.config.hidden_size, num_layers=self.config.num_layers).cuda()
    ...
    def train(self):
        ...
        for epoch_i in trange(self.config.n_epochs, desc='Epoch', ncols=80):
            ...
            save_epochs = [0,9,19,29,39,49]
            if epoch_i in save_epochs:
    ...
    def evaluate(self, epoch_i):
        ...
        for video_tensor, video_name in tqdm(self.test_loader, desc='Evaluate', ncols=80, leave=False):
            ...
            print(f"Saving path is {score_save_path}")
