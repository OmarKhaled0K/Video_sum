...
path = r"E:\Artificial_Intelligence\Video Summarization\Code\HPC\SUM-GAN-AED\exp1\summe\results\split0" # path to the json files with the computed importance scores for each epoch
results = listdir(path)
results=results[0:-1]
print(results)
if 'f_scores.txt' in results:
    results.remove('f_scores.txt')
results.sort(key=lambda video: int(video[6:-5]))
PATH_SumMe = '../SumMe/eccv16_dataset_summe_google_pool5.h5'
...
for epoch in results:
    ...
    with open(path+'/'+epoch) as f:
        ...
        print(f'f = {f}')
        print('-'*40)
        #if f.name.endswith("summe_4.json") or f.name.endswith("summe_29.json"):
        #    continue
